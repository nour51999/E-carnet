...
@section('content')
    ...
    <form action="{{ route('abs.search') }}" method="GET" class="formr card-body c">
...
<form method="POST" action="{{route('absences.store')}}" enctype="multipart/form-data" class="form2 c">
    @csrf
...
<h3 class="h" style="color:#976cd8; text-align:center;">Absences Et Retards</h3>
...
<tbody>
    ...
    <td > <select  name="data[{{ $index }}][abs_id]"   required>
    ...
    <td>
        <select  name="data[{{ $index }}][abr]"  >
            <option value="abcence">Absence</option>
            <option value="retards">Retads</option>
            <option value="presence">Presence</option>
        </select>     
</td>
    <td> <select  name="data[{{ $index }}][jfs]" >
        <option value="malade">malade</option>
        <option value="voyage">voyage</option>
        <option value="sans justification">sans justification</option>
        <option value="décés">décés</option>
        <option value="/">/</option>
        <option value="circulation de route">circulation de route</option>
